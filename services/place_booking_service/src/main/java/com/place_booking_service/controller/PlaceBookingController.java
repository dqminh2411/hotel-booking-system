...
public class PlaceBookingController {
    ...
    public ApiResponse<?> placeBooking(@Valid @RequestBody PlaceBookingRequest placeBookingRequest) {
        ...
        DuplicateRequestResult result = helpler.isFirstRequest(placeBookingRequest.getUserId(), hashRequest, UUID.randomUUID());

        UUID bookingId = result.bookingId();
        ...
        if (!result.isDuplicated()) {
            ...
            bookingId = placeBookingService.startSaga(placeBookingRequest, user, hotelAndRoomTypes.hotel(),
                    result.bookingId(), hashRequest);
        }
